...
# SO turns out i was solving part 2 for part 1, so the part i was struggling with on part 1 was likely a misunderstanding of the task/edge case -> leading to chatgpt use.
...
def findMaximumTrailScore(trailheads):
    ...
    for start in trailheads:
        ...
        total += checkPosition(current_coords, 0, set())

    return total

# Outside recursive function to reduce memory trace on stack as argc <. 
# Cannot go diagonal
...
def checkPosition(current_coords, counter, visited):
    # ChatGPT fixed that I was visiting the same nodes on the same path more then once.
    if tuple(current_coords) in visited:
        return counter
    visited.add(tuple(current_coords))
    ...
    for move in surrounding_positions:
        ...
        if map[next_coords[0]][next_coords[1]] == (map[current_coords[0]][current_coords[1]] + 1):
            # Move to next co-ord
            counter = checkPosition(next_coords, counter, visited)
